...
public class StackersPlanning extends BaseTest {
	...
	public StackersPlanning() throws FileNotFoundException {
	...
	WebElement addStackersPlanning;
	...
	WebElement saveStackersPlanning;

	@FindBy(xpath = "//li[@title='SP01']/span")
	WebElement equipmentTypeOneCancel;


	@FindBy(xpath = "//li[@title='SP02']/span")
	WebElement equipmentTypeTwoCancel;
	...
	WebElement stackersFinalBookingReportBookingDate;
	...
	@FindBy(xpath = "//select[@name='equipmentTypeName']")
	WebElement equipmentTypeSelection;
	

	@FindBy(xpath = "(//*[contains(@class,'selection__arrow')])[2]")
	WebElement equipmentTypeSelectionArrow;
	
	@FindBy(xpath = "//*[contains(@id,'cmbEquipmentTypeName-container')]/span")
	WebElement equipmentTypeSelectionCancelArrow;
	

	@FindBy(xpath = "//*[text()='Select Equipment Type']")
	WebElement equipmentTypeClick;
	

	@FindBy(xpath = "//select[@placeholder='Select Equipment Type']")
	WebElement equipmentTypeSelect;
	
	@FindBy(xpath = "//input[@type='search']")
	WebElement equipmentTypeSearchIput;
	...
	WebElement btnSearchStackersFinalbookingReport;
...
//	@FindBy(xpath = "(//*[@id=\"dpDailyPlanningTaskSummary\"]//div[@class='centered_rowheader_inner']//div)["+i+"]")
	...
	public void createStackersPlanning() throws Exception {

		System.out.println("Stackers Planning");
		Date d = new Date();
		...
		Thread.sleep(2000);
		dtPlanning.sendKeys("05-08-2023");
		...
		webAction.clickUsingJavaScript(btnSearchStackersPlanning);
		...
		webAction.clickUsingJavaScript(equipmentTypeTwoCancel);	
		webAction.clickUsingJavaScript(equipmentTypeOneCancel);
		
		webAction.clickUsingJavaScript(btnSearchStackersPlanning);
		
		String equipmentType = "SP01";
		
		List<WebElement> equipmentTypesel = equipmentTypeSelection
				.findElements(By.xpath("//select[@name='equipmentTypeName']/option"));
		for (WebElement we : equipmentTypesel) {
			Thread.sleep(500);
			// we.sendKeys(Keys.ARROW_DOWN);
			// if(shiftBandNew.equals(we.getText()))
			if (equipmentType.equals(we.getText()))
				we.click();
			// Thread.sleep(500);
		}
		
		
		//equipmentTypeSearchIput.sendKeys("SP01");
		//equipmentTypeSearchIput.sendKeys(Keys.ENTER);
	...
	//	webAction.clickUsingJavaScript(btnSearchStackersPlanning);
		...
		/*
		 * reportUtils.addInfo("Before Stackers Planning Status", methodName);
		 * reportUtils.addInfo("####Equipment Summary####", methodName);
		 * reportUtils.addInfo("Equipment Name     :"+equipmentName.getText(),methodName
		 * ); reportUtils.addInfo("Required Equipment :"+requiredEquipment.getText(),
		 * methodName); reportUtils.addInfo("Available Equipment:"
		 * +availableEquipment.getText(),methodName);
		 * webAction.clickUsingJavaScript(skillSummary); Thread.sleep(4000);
		 * reportUtils.addInfo("####Skill Summary####", methodName);
		 * reportUtils.addInfo("Skill code        :"+skillCodeOne.getText(),methodName);
		 * reportUtils.addInfo("Required  Skill   :"+requiredSkillCodeOne.getText(),
		 * methodName);
		 * reportUtils.addInfo("Available Skill   :"+availableSkillCodeOne.getText(),
		 * methodName);
		 * reportUtils.addInfo("Required Manpower :"+requiredManPowerSkillCodeOne.
		 * getText(),methodName);
		 * 
		 * reportUtils.addInfo("Skill Two         :"+skillCodeTwo.getText(),methodName);
		 * reportUtils.addInfo("Required  Skill   :"+requiredSkillCodeTwo.getText(),
		 * methodName);
		 * reportUtils.addInfo("Available Skill   :"+availableSkillCodeTwo.getText(),
		 * methodName);
		 * reportUtils.addInfo("Required Manpower :"+requiredManPowerSkillCodeTwo.
		 * getText(),methodName);
		 */
		webAction.scrollDown300();
		Thread.sleep(2000);
		webAction.scrollUp();
		webAction.waitUntilElementIsClickable(addStackersPlanning);
		webAction.clickUsingJavaScript(addStackersPlanning);
		
		System.out.println("Entered into Adding Stackers Planning");
		Thread.sleep(9000);
	//	webAction.click(equipmentTypeSelection);
		
//String equipmentType = "SP01";
		
		if(resue.isElementPresent(equipmentTypeSelectionCancelArrow))
		{
			webAction.click(equipmentTypeSelectionCancelArrow);
			  Thread.sleep(2000); 
			  equipmentTypeSearchIput.sendKeys("SP01");
			  equipmentTypeSearchIput.sendKeys(Keys.ENTER);
		...
		else
		{
			System.out.println("No Such a element");
			  Thread.sleep(2000); 
			webAction.click(equipmentTypeSelectionArrow);
			  Thread.sleep(2000); 
			  equipmentTypeSearchIput.sendKeys("SP01");
			  equipmentTypeSearchIput.sendKeys(Keys.ENTER);
		...
		/*
		 * List<WebElement> equipmentTypesele = equipmentTypeSelection
		 * .findElements(By.xpath("//select[@name='equipmentTypeName']/option")); for
		 * (WebElement we : equipmentTypesele) { Thread.sleep(500); //
		 * we.sendKeys(Keys.ARROW_DOWN); // if(shiftBandNew.equals(we.getText())) if
		 * (equipmentType.equals(we.getText())) we.click(); // Thread.sleep(500); }
		 */	
		
		//  handler.clickUsingActionsMethod(equipmentTypeSelection);
		...
		webAction.clickUsingJavaScript(saveStackersPlanning);
	...
	/*
	 * reportUtils.addInfo("After Stackers Planning Status", methodName);
	 * reportUtils.addInfo("####Equipment Summary####", methodName);
	 * reportUtils.addInfo("Equipment Name     :"+equipmentName.getText(),methodName
	 * ); reportUtils.addInfo("Required Equipment :"+requiredEquipment.getText(),
	 * methodName); reportUtils.addInfo("Available Equipment:"
	...
	public void viewDeployment() throws Exception {
		...
		Thread.sleep(500);
		Thread.sleep(500);
		stackersFinalBookingReportBookingDate.clear();
		stackersFinalBookingReportBookingDate.sendKeys(date);
		Thread.sleep(1000);
		...
		List<WebElement> equipType = equipmentTypeSelection
				.findElements(By.xpath("(//select[@placeholder='Select Equipment Type'])[2]/option"));
		for (WebElement we : equipType) {
			Thread.sleep(500);
			// we.sendKeys(Keys.ARROW_DOWN);
			// if(shiftBandNew.equals(we.getText()))
			if (equipmentType.equals(we.getText()))
		...
		webAction.clickUsingJavaScript(btnSearchStackersFinalbookingReport);
		Thread.sleep(2000);
